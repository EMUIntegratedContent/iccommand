	...
	<div>
		...
		<div class="tab-content" id="linkTabsContent">
			<div
				...
				id="broken"
				role="tabpanel"
				aria-labelledby="broken-links-tab"
			>
				<div>
					<label for="selectedredir" class="sr-only"
						>Search broken redirects</label
					>
					<VueMultiselect
				...
				<div v-if="!loadingRedirectsOfBrokenLinks" class="table-responsive">
					<table class="table table-hover table-sm">
						...
						<tbody>
							<tr
								v-for="redirect in brokenLinks"
								:id="redirect.id"
								:key="`broken-${redirect.id}`"
							>
								<td>
									<a
										:href="'https://www.emich.edu' + redirect.fromLink"
										target="_blank"
										title="Go to this Eastern Michigan University page."
										>{{ redirect.fromLink }}</a
									>
								</td>
								<td>
								...
								<td>
									<a v-if="userCanEdit" :href="'/redirects/' + redirect.id"
				...
				<external-paginator
					...
					@pageChanged="handleBrokenItemsPageChanged"
				></external-paginator>
			</div>
			<div
				class="tab-pane fade"
				id="shortened"
				role="tabpanel"
				aria-labelledby="shortened-links-tab"
			>
				<div>
					<label for="selectedshort" class="sr-only"
						>Search shortened redirects</label
					>
					<VueMultiselect
				...
				<div v-if="!loadingRedirectsOfShortenedLinks" class="table-responsive">
					<table class="table table-hover table-sm">
						...
						<tbody>
							<tr
								v-for="redirect in shortenedLinks"
								:id="redirect.id"
								:key="`shortened-${redirect.id}`"
							>
								<td>
									<a
										:href="'https://www.emich.edu' + redirect.fromLink"
				...
				<external-paginator
					...
					@pageChanged="handleShortenedItemsPageChanged"
				></external-paginator>
			</div>
		</div>
	</div>
...
export default {
	...
	data: function () {
		return {
			...
			brokenRedirectsItemsPerPage: 100,
			totalBrokenRedirects: 0,
			shortenedRedirectsCurrentPage: 1,
			shortenedRedirectsItemsPerPage: 100,
	...
	methods: {
		...
		fetchRedirects: function (type) {
			...
			axios
				.get(
					`/api/redirects/list?page=${currentPage}&limit=${itemsPerPage}&type=${type}`
				)
					...
					switch (error.response.status) {
						case 403:
							self.apiError.message =
								"You do not have sufficient privileges to retrieve redirects."
							break
						case 404:
		...
		searchRedirects: function (type) {
			...
			if (searchTerm.length > 2) {
				/* Ajax (Axios) Submission */
				axios
					.get(`/api/redirects/search?searchterm=${searchTerm}&type=${type}`)
					.then(function (response) {
						// Success.
						if (type === "broken") {

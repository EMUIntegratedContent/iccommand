<template>
	<div>
		...
		<div
			...
			role="alert"
		>
			This redirect has been deleted. You will now be redirected to the redirect
			list page.
		</div>
		...
		<div v-if="isDataLoaded === true && isDeleted === false && is404 === false">
			...
			<div class="btn-group" role="group" aria-label="form navigation buttons">
				<button v-if="newForm" class="btn btn-info pull-left" @click="goBack()">
					<i class="fa fa-chevron-circle-left"></i> Step 1
				</button>
				<button
			...
			<div class="pt-2" id="redirectItemTabContent">
				<VeeForm
					...
					:validation-schema="redirectSchema"
				>
					<fieldset>
					...
					>
						<p v-if="isSaveFailed" class="red">
							Error saving this redirect. {{ apiError.message }}
						</p>
				...
				<div
					v-if="!itemExists && record.itemType == 'redirect of broken link'"
					...
					<table class="table table-hover table-sm">
						...
						<tbody>
							<tr v-for="uncaught in uncaughtsInRecommended" :id="uncaught.id">
								...
								<td>
									<i
										class="fa fa-plus-square pointer fa-2x pr-4 green"
										@click="setFromLink(uncaught.link)"
									></i>
									<i
										class="fa fa-times-circle pointer fa-2x red"
										@click="removeFromRecommended(uncaught)"
									></i>
								</td>
							</tr>
						</tbody>
	...
	computed: {
		...
		userCanEdit: function () {
			return (this.itemExists && this.permissions[0].user) ||
				(!this.itemExists && this.permissions[0].user)
				? true
				: false
		}
	},
					...
					switch (error.response.status) {
						case 403:
							self.apiError.message =
								"You do not have sufficient privileges to retrieve uncaught items."
							break
						case 404:

<template>
	<div>
		...
		<div class="form-group row mb-3">
			<label for="cycleSelect" class="col-sm-2 col-form-label"
				><strong>Cycle:</strong></label
			>
			<div class="col-sm-4">
				<select
					id="cycleSelect"
		...
		<div v-if="!loading" class="table-responsive">
			...
			<p v-if="links.length === 0" class="text-muted text-center">
				No links found for this cycle.
			</p>
		</div>
		<div v-else>
				...
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title">
							CSV Upload - Links for {{ selectedCycleName }}
						</h5>
						<button
							type="button"
							class="close"
					...
					<div class="modal-body">
						<p>
							Upload a CSV file with columns: <code>degree_name</code>,
							<code>link</code>
						</p>
						...
						<div v-if="uploadStatus === 'success'" class="alert alert-success">
							{{ uploadMessage }}
						</div>
						<div v-if="uploadStatus === 'failed'" class="alert alert-danger">
							{{ uploadMessage }}
						</div>
					</div>
		...
		uploadCsv() {
			...
			axios
				...
				.catch(function (error) {
					...
					self.uploadMessage = error.response
						? error.response.data
						: "Upload failed."
				})
		}

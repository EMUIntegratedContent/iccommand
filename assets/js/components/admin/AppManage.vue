      ...
      <div v-for="(user, index) in paginatedUsers" class="col-xs-12 col-sm-6 col-md-4 pl-4 pb-2">
        <div class="card card-accent card-accent-green"
          ...
          <div class="card-header"
            ...
            {{ user.username }}
            <button v-if="currentUser != user.username" type="button" @click="openDeleteModal(user, index)"
                    class="close pull-right"><span aria-hidden="true">&times;</span></button>
          </div>
          <div class="card-body">
            ...
            <template v-else>
              ...
              <template v-else>
                ...
                <div v-if="isCardModified(index) && isCardError(index)" class="alert alert-danger fade show mt-4"
                     role="alert">
                  There was a problem saving this user.
                </div>
                <div class="user-card-buttons pt-4" :class="{'d-none' : !isCardModified(index)}">
                  <button class="btn btn-success" type="submit" @click="saveUser(user, index)"><i
                      class="fa fa-save"></i></button>
                  <button class="btn btn-default ml-2" @click="resetUser(user.id, index)"><i class="fa fa-undo"></i>
                  </button>
                </div>
              </template>
            </template>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Revoke {{ deleteModalData.user ? deleteModalData.user.username : '' }}'s
              privileges</h5>
          ...
          <div class="modal-body">
            <p>Are you sure you want to revoke {{ deleteModalData.user ? deleteModalData.user.username : '' }}'s
              privileges for {{ appName }}? Type the word <strong>"delete"</strong> to confirm.</p>
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-danger" data-dismiss="modal"
                    @click="removeUserFromApp(deleteModalData.user, deleteModalData.index)"
                    :disabled="deleteConfirm != 'delete'">Revoke
            </button>
          </div>
        </div>
      </div>
...
export default {
  ...
  components: {Heading, Paginator, VueMultiselect},
  props: {
    appName: {
      type: String,
      required: true,
        ...
        data: user
          ...
          .then(function (response) {
            if (isDelete) {
              ...
              // remove from the deleted cards array after 2 seconds

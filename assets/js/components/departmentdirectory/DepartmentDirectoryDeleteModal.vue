    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Delete {{ request.requestType }} request</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p>Are you sure you want to delete this {{ request.requestType }} request? Type the word <strong>"delete"</strong> to confirm.</p>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-danger" data-dismiss="modal" @click="delItem" :disabled="deleteConfirm != 'delete'">Delete request</button>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    delItem: function() {
      ...
      if(this.deleteConfirm == 'delete'){
        ...
        axios.delete('/api/multimediarequests/' + this.request.id)

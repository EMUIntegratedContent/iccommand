<template>
	<div>
		...
		<div>
			<p>
				This is a list of all programs that are in the current year's course
				catalog, divided into undergraduate and graduate views. The list does
				NOT include previous year's programs.
			</p>
		</div>
		<ul class="nav nav-tabs mb-3" id="programTabs" role="tablist">
			<li class="nav-item">
				<a
					...
					<span v-if="!loadingUgPrograms" class="badge badge-primary">{{
						totalUgPrograms
					}}</span>
			...
			<li class="nav-item">
				<a
					...
					<span v-if="!loadingGradPrograms" class="badge badge-primary">{{
						totalGradPrograms
					}}</span>
					<span v-else><i class="fa fa-spinner"></i></span
				></a>
		...
		<div class="tab-content" id="programTabsContent">
			...
			>
				<div>
					<label for="ugselectedprog" class="sr-only"
						>Search undergrad catalog</label
					>
					<VueMultiselect
					...
					<table class="table table-hover table-sm">
						...
						<tbody>
							...
							>
								...
								<td>{{ ugp.id }}</td>
								<td>
									<a v-if="userCanEdit" :href="'/programs/' + ugp.id"
			...
			>
				<div>
					<label for="gradselectedprog" class="sr-only"
						>Search graduate catalog</label
					>
					<VueMultiselect
					...
					<table class="table table-hover table-sm">
						...
						<tbody>
							...
							>
								...
								<td>{{ grad.url }}</td>
								<td>
									<a v-if="userCanEdit" :href="'/programs/' + grad.id"
										><font-awesome-icon icon="fa-solid fa-pen-to-square"
									/></a>
								</td>
							</tr>
...
export default {
	...
	methods: {
		...
		fetchPrograms: function (catalog) {
			...
			axios
				.get(
					`/api/programs/list?page=${currPage}&limit=${currItemsPerPage}&catalog=${catalog}`
				)
					...
					switch (error.response.status) {
						case 403:
							self.apiError.message =
								"You do not have sufficient privileges to retrieve programs."
							break
						case 404:
		...
		searchPrograms: function (catalog) {
			...
			if (searchTerm.length > 2) {
				...
				axios
					.get(
						`/api/programs/search?searchterm=${searchTerm}&catalog=${catalog}`
					)
					.then(function (response) {
						// Success.
						if (catalog === "undergraduate") {

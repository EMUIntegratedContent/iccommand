<template>
	<div>
		...
		<div
			...
			role="alert"
		>
			This website has been deleted. You will now be redirected to the websites
			list page.
		</div>
		...
		<div v-if="isDataLoaded === true && isDeleted === false && is404 === false">
			...
			<div class="btn-group" role="group" aria-label="form navigation buttons">
				<button
					v-if="siteExists && permissions[0].edit"
					type="button"
			...
			<div class="pt-2" id="websiteTabContent">
				...
				>
					<div class="form-group">
						<label
							>Program website
							<span v-if="isEditMode" class="red">*</span></label
						>
						<Field
							name="programUrl"
							type="text"
					...
					<div class="form-group">
						<label
							>Program name <span v-if="isEditMode" class="red">*</span></label
						>
						<Field
							name="programName"
							type="text"
					...
					>
						<p v-if="isSaveFailed" class="red">
							Error saving this website. {{ apiError.message }}
						</p>
...
export default {
	...
	computed: {
		...
		userCanEdit: function () {
			return (this.siteExists && this.permissions[0].create) ||
				(!this.siteExists && this.permissions[0].edit)
				? true
				: false
		}
	},
	...
	methods: {
		...
		fetchWebsite: function (websiteId) {
			const self = this
			axios
				.get("/api/programs/websites/" + websiteId)

  ...
  <div>
    ...
    <div v-if="isDataLoaded === true && isDeleted === false && is404 === false">
      <heading>
        ...
        <span slot="title">{{ record.firstName }} {{
            record.lastName
          }}'s {{ requestType == 'publication' ? 'marketing materials' : requestType }} request </span>
      </heading>
      ...
      <VeeForm class="form" v-slot="{ submitForm, errors, meta }" @submit="submitMmr" :validation-schema="mmrSchema">
        <div class="row">
          <div class="col-md-8">
            <fieldset>
              <!-- COMMON FIELDS -->
              ...
              <div class="form-group">
                ...
                <input
                    ...
                    type="text"
                    class="form-control"
                    :class="{ 'is-invalid': errors.phone, 'form-control-plaintext': !userCanEdit || !isEditMode }"
                    :readonly="!userCanEdit || !isEditMode"
              ...
              <div class="form-group">
                ...
                <input
                    ...
                    type="text"
                    class="form-control"
                    :class="{ 'is-invalid': errors.department, 'form-control-plaintext': !userCanEdit || !isEditMode }"
                    :readonly="!userCanEdit || !isEditMode"
            ...
            <fieldset>
              ...
              <legend>{{ capitalize(requestType) }} request information</legend>
              ...
              <template v-if="requestType == 'photo'">
                <div class="row">
                  ...
                  <div class="form-group col-md-5">
                    ...
                    <input
                        ...
                        type="text"
                        class="form-control"
                        :class="{ 'is-invalid': errors.location, 'form-control-plaintext': !userCanEdit || !isEditMode }"
                        :readonly="!userCanEdit || !isEditMode"
                    ...
                    <template v-if="isEditMode && record.photoRequestType">
                      ...
                      <div class="form-check">
                        <input
                            ...
                            :class="{ 'is-invalid': errors.photoRequestType, 'form-control-plaintext': !userCanEdit || !isEditMode }"
                            :disabled="!isEditMode"
                    ...
                    <template v-if="isEditMode">
                      ...
                      <div class="form-check">
                        <input
                            ...
                            value="web"
                            class="form-check-input"
                            :class="{ 'is-invalid': errors.intendedUse, 'form-control-plaintext': !userCanEdit || !isEditMode }"
                ...
                <div class="row">
                  <div class="form-group col-md-6">
                    ...
                    <div v-else>
                      {{ commentDateFormat(record.startTime) }}
                    </div>
                  </div>
                  <div class="form-group col-md-6">
                    ...
                    <div v-else>
                      {{ commentDateFormat(record.endTime) }}
                    </div>
                  </div>
                </div>
              </template>
              ...
              <template v-if="requestType == 'video'">
                ...
                <div class="form-group">
                  ...
                  <div v-else>
                    {{ dateOnlyFormat(record.completionDate) }}
                  </div>
                </div>
              </template>
              ...
              <template v-if="requestType == 'publication'">
                <div class="form-group">
                  ...
                  <div v-if="isEditMode" class="form-group">
                    <label for="pubRequestType">Materials type*</label>
                    <Field
                    ...
                    >
                      <VueMultiselect
                          ...
                          style="padding:0"
                          id="pubRequestType"
                          :class="{'is-invalid': errors.publicationRequestType }"
                      >
                      </VueMultiselect>
                    </Field>
                    <div class="invalid-feedback">
                      {{ errors.publicationRequestType }}
                    </div>
                  </div>
                  <div v-else>
                    <p v-if="record.publicationRequestType != null">Materials needed:
                      {{ record.publicationRequestType.requestType }}</p>
                  ...
                  <template v-if="isEditMode">
                    <div class="form-check">
                      <input
                          ...
                          value="1"
                          class="form-check-input"
                          :class="{ 'is-invalid': errors.isPhotographyRequired, 'form-control-plaintext': !userCanEdit || !isEditMode }"
                ...
                <div class="row">
                  ...
                  <div v-if="userCanEmail" class="col-sm-12 status-container">
                    ...
                    <div v-if="record.assignee && !reminderEmailStatus.isSent && !reminderEmailStatus.isError">
                      ...
                      <div class="form-group">
                        <textarea type="text" class="form-control" v-model="reminderEmailBody"
                                  :placeholder="'Optional custom message to ' + record.assignee.firstName"></textarea>
                      </div>
                      <button type="button" class="btn btn-sm btn-info" @click="sendAssigneeEmailNotification">Notify
                        {{ record.assignee.firstName }}
                      </button>
                    </div>
                      ...
                      <div class="status-note-container">
                        ...
                        <aside v-for="(note, index) in record.statusNotes" class="status-note">
                          ...
                          <template v-if="note.id">
                            <p>{{ note.note }}</p>
                            <p class="text-right note-meta">{{ note.created_by }} at {{
                                commentDateFormat(note.created)
                              }}
                              <button type="button" class="btn btn-sm btn-outline-danger"
                                      @click="removeStatusNote(note)">Discard
                              </button>
                            </p>
                          </template>
                          <!-- New notes -->
                          <template v-else>
                            <textarea v-model="note.note" class="form-control"
                                      placeholder="What would you like to say?"></textarea>
                            <p class="text-right note-meta">
                              <button type="button" class="btn btn-sm btn-outline-danger"
                                      @click="removeStatusNote(note)">Discard
                              </button>
                            </p>
              ...
              <template v-else>
                ...
                <div class="form-group">
                  <p><strong>Assigned to
                    &ndash; </strong>{{ record.assignee != null ? record.assignee.displayStr : 'nobody' }}</p>
                </div>
                <template v-if="record.statusNotes.length > 0">
                  <p><strong>Notes about this request.</strong></p>
                  <div class="status-note-container">
...
export default {
  ...
  components: {MultimediaRequestDeleteModal, Heading, VueMultiselect, NotFound, Yup, Field, VeeForm, ErrorMessage, Flatpickr},
  props: {
    itemId: {
      type: String,
      required: false
  ...
  computed: {
    ...
    mmrSchema() {
      ...
      if(this.requestType == 'headshot') {
        yupObj['timeSlot'] = Yup.object().required().label('Time slot ')
      }
      if(this.requestType == 'publication') {
        yupObj['publicationRequestType'] = Yup.object().required().label('Materials type ')
      }
  ...
  methods: {
    ...
    endTimeChanged: function (dateObj, dateStr) {
      this.flatpickrStartTimeConfig.maxDate = dateStr
    },
    ...
    fetchPublicationTypes: function () {
      ...
      axios.get(url)
          ...
          // fail
          .catch(function (error) {
            console.log("ERROR FETCHING PUB TYPES!")
          })
    },
    ...
    startTimeChanged: function (dateObj, dateStr) {
      this.flatpickrEndTimeConfig.minDate = dateStr
    },

...
**The final audit (section 7) found one more High security issue and one High bug.** An authenticated SQL injection exists in the Programs list endpoints (S21). An expired session makes every API save report success while silently saving nothing. Every redirect visit erases that redirect's "changed by" value in the admin list, because of a Blameable listener. It also found that **production is serving a 15.7 MB development build of the frontend today**, because production deploys by pulling `master` and `master` has a dev build committed. Production error pages render blank, and staging runs in debug mode.
...
- Versioning (`enableVersioning()`) is commented out, so there is no cache-busting today: the file is always `app.js`, and browsers can keep using a cached old copy after a deploy. **Decision (2026-09-23): turn versioning on** (Phase 0).
...
- **Production is serving a development build today** (production pulls `master`, and `master` has the 15.7 MB dev build committed). Set `__VUE_PROD_DEVTOOLS__: false`, **uncomment `.enableVersioning()` in `webpack.config.js`** (team decision 2026-09-23), run `npm run build`, and commit the production build to `master`. With versioning on, each build writes hashed names such as `app.3f9a1c2b.js` and records them in `public/build/entrypoints.json` and `manifest.json`; the existing `encore_entry_script_tags`/`encore_entry_link_tags` calls read those files, so no template changes are needed, and every deploy is picked up by browsers immediately. Side effects to expect:
  - Every build renames the files, so each rebuild commit deletes the old hashed files and adds new ones. Two branches that both rebuild will conflict on `entrypoints.json` and `manifest.json`; resolve by rebuilding after merging rather than hand-editing. This is the strongest argument for option (b) in decision 1.
  - A page opened before a deploy may request the old hashed file, which `git pull` has removed; a reload fixes it.
...
| 2 | ICCommand | Make both counters atomic and listener-free: `UPDATE ... SET visits = visits + 1` through DBAL, and `INSERT ... ON DUPLICATE KEY UPDATE` for the uncaught POST with a length cap (S18). This also stops the Gedmo Blameable listener from erasing the redirect's "changed by" value on every visit (7.1). Remove the rate limiter subscriber, its `rate_limiter.yaml` entry, its manual service definition and its `bind` (S10). |
...
8. **Operations:** adopt the deploy script, scheduled backups of both databases and the uploads directories, one full restore rehearsal, and a runbook (7.8). Confirm how logs are rotated, and whether staging should keep a debug mode behind access control. Resolved 2026-09-23: production timezone is `America/Detroit`; staging runs `APP_ENV=staging` in debug; production deploys `master` via `git pull`; deploys and backups are manual; prod has never been restored from backup; the uncaught counter URL is fixed; the redirect counter works (but erases "changed by", 7.1); prod data in staging and local is an accepted risk.
...
|---|---|---|---|
| Med | **Every redirect visit erases the redirect's "changed by" value.** The counter itself works (confirmed 2026-09-23). | `Redirect.php:76` puts `Gedmo\Blameable(on: "update")` on `contentChanged`. The anonymous `PUT external/redirect` has no logged-in user, and Gedmo writes the actor unconditionally (`AbstractTrackingListener::updateField`), so each visit sets `contentChanged` to NULL (or `''` if the live column is NOT NULL and `sql_mode` is non-strict). `RedirectList.vue:115,204` displays this column, so for any redirect that gets traffic the admin list loses who last edited it. The `updated` timestamp is also bumped on every visit, so it no longer means "last edited". The Phase 0a shared token is not a Symfony user, so it does not fix this. | DBAL `UPDATE redirect SET visits = visits + 1, last_visit = NOW()` bypassing ORM listeners (Phase 0a step 2). Historic values are already lost for visited redirects. |
...
- **Production has never been restored from a backup** (confirmed 2026-09-23). Prod dumps are routinely loaded into staging and local databases, which shows the dumps are loadable, but not that a full recovery works: the `dps` database, uploaded images, `.env` and server config also have to come back, and nobody has timed it.
- **Production data is copied to staging and developer machines. Accepted risk, no action planned (team decision 2026-09-23).** Prod dumps include user emails and phone numbers, photo-request requester details and crime-log records. This is recorded for awareness only. Note that the separate staging debug-mode fix (7.4) and profiler fix (S14) remain in the plan and also limit exposure of this data.
...
2. **Scheduled backups** via cron on the server: nightly `mysqldump --single-transaction` of both the `ic` and `dps` databases plus an `rsync`/`tar` of `public/uploads`, copied off the server and kept for a set period. Once, do a full rehearsal: rebuild a scratch server from the backups alone (code from git, both databases, uploads, `.env`) and write down every step and how long it took. That becomes the restore section of the runbook.

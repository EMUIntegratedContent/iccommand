...
8. **Operations:** adopt the deploy script, scheduled backups of both databases and the uploads directories, one full restore rehearsal, and a runbook (7.8). Decide whether prod data loaded into staging and local should be sanitised. Confirm how logs are rotated, and whether staging should keep a debug mode behind access control. Resolved 2026-09-23: production timezone is `America/Detroit`; staging runs `APP_ENV=staging` in debug; production deploys `master` via `git pull`; deploys and backups are manual; prod has never been restored from backup; the uncaught counter URL is fixed; the redirect counter works (but erases "changed by", 7.1).
...
- **Production data is copied to staging and developer machines.** That includes user emails and phone numbers, photo-request requester details and crime-log records. Staging runs in debug mode with the profiler exposed (S14, 7.4), so any request data there is visible to anyone who can reach it. Consider a small sanitising script that blanks personal fields after loading a dump into non-production databases, and fix staging's debug mode first.
